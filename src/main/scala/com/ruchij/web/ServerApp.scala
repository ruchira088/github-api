...
import com.ruchij.web.routes.{IndexRoute, MissingApiTokenRoute}
  ...
  def main(args: Array[String]): Unit =
  {
    ...
    val serverBinding =
      for {
        ...
        server <- Http().bindAndHandle(IndexRoute(gitHubService), DefaultConfig.SERVER_ADDRESS, httpPort())
      }
      yield server
    ...
    serverBinding.onComplete {
      ...
      case Failure(GitHubApiTokenUndefinedException) => {
        System.err.println(GitHubApiTokenUndefinedException)

        Http().bindAndHandle(MissingApiTokenRoute(), DefaultConfig.SERVER_ADDRESS, httpPort())
            .map(_ => println(s"Serving ${StatusCodes.BadGateway} to incoming requests..."))
      }
